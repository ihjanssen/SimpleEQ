...
void SimpleEQAudioProcessor::updatePeakFilter(const ChainSettings& chainSettings) {
    auto peakCoefficients =
        juce::dsp::IIR::Coefficients<float>::makePeakFilter(getSampleRate(), chainSettings.peakFreq, chainSettings.peakQuality, juce::Decibels::decibelsToGain(chainSettings.peakGainInDecibels));
...
void SimpleEQAudioProcessor::updateLowCutFilters(const ChainSettings& chainSettings) {
    ...
    auto cutCoefficient = juce::dsp::FilterDesign<float>::designIIRHighpassHighOrderButterworthMethod(chainSettings.lowCutFreq, getSampleRate(), 2 * (chainSettings.lowCutSlope + 1)); //butterworth requires last parameter of 2,4,6,8 for 12,24,36,48 db slopes, so our slope selection +1 *2 will yield the correct number
...
void SimpleEQAudioProcessor::updateHighCutFilters(const ChainSettings& chainSettings) {
    ...
    auto highCutCoefficient = juce::dsp::FilterDesign<float>::designIIRLowpassHighOrderButterworthMethod(chainSettings.highCutFreq, getSampleRate(), 2 * (chainSettings.highCutSlope + 1)); //butterworth requires last parameter of 2,4,6,8 for 12,24,36,48 db slopes, so our slope selection +1 *2 will yield the correct number
